...
const fetchCoordsByIP = function (ip, callback){
  request(`http://ipwho.is/${ip}`, function (error, response, body){
    ...
    const longitude = parseBody.longitude
...
const fetchMyIP = function (callback) {
  ...
  request(`https://api.ipify.org?format=json`, function (error, response, body) {
    ...
    let passingJson = JSON.parse(body)
    const ip = passingJson.ip
...
//ISS functions here. call this function by exporting this function in index.js
 const fetchISSFlyOverTimes = function(coords, callback) {
  request(`https://iss-flyover.herokuapp.com/json/?lat=${coords.latitude}&lon=${coords.longitude}`, function (error, response, body) {
    ...
    if (response.statusCode !== 200) {
      const msg = `Failed fetching flyover times ${response.statusCode}`;
      callback(Error(msg), null);
      return;
    }

    let passingJson = JSON.parse(body)
    const passingData = passingJson.response
    return callback(null, passingData)
})
...
//fetchCoordsByIP("174.91.156.49", (ip) => console.log(ip))
